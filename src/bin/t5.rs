...
impl Context {
    fn a(& self) {
        ...
        self.t1.store(1, Ordering::SeqCst);
    }
    fn b(& self){
        while self.flag.load(Ordering::Acquire) == 0{}

        self.t2.store(1, Ordering::SeqCst);
    }
    fn c(& self){
        while self.flag.load(Ordering::Acquire) == 0{}

        while self.t1.load(Ordering::SeqCst) == 0{}
        if self.t2.load(Ordering::SeqCst) == 1 {
    ...
    fn d(& self){
        ...
        while self.t2.load(Ordering::SeqCst) == 0{}
        if self.t1.load(Ordering::SeqCst) == 1 {
            self.z.fetch_add(1, Ordering::AcqRel);
        }
    }
}
fn run(i:i32){
    ...
    t3.join().unwrap();
    t4.join().unwrap();
    let v = context_arc.z.load(Ordering::SeqCst);
    println!("i={}, v={}", i, v);
}
fn main(){
    for i in 0..100 {
